...
public:
  ...
  AirportCounter(TIterator begin, TIterator end)
  {
      ...
      {
          airportToFlights[airportIndex].first = static_cast<TAirport>(airportIndex);
      }
      for (auto flight = begin; flight != end; flight++)
      {
          //airportToFlights[static_cast<uint32_t>(*flight)].first = *flight;
          airportToFlights[static_cast<uint32_t>(*flight)].second++;
      }
  }
  ...
  {
      return airportToFlights[static_cast<uint32_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport)
  {
      airportToFlights[static_cast<uint32_t>(airport)].second++;
  }
  ...
  {
      airportToFlights[static_cast<uint32_t>(airport)].second = 0;
  }
