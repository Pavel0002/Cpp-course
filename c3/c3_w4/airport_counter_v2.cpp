...
public:
  ...
  {
      for (size_t airportIndex = 0; airportIndex < airportsCount; airportIndex++)
      {
          airportToFlights[airportIndex] = 0;
      }
      for (auto flight = begin; flight != end; flight++)
      {
          //airportToFlights[static_cast<uint32_t>(*flight)].first = *flight;
          airportToFlights[static_cast<uint32_t>(*flight)]++;
      }
  }
  ...
  {
      return airportToFlights[static_cast<uint32_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport)
  {
      airportToFlights[static_cast<uint32_t>(airport)]++;
  }
  ...
  {
      airportToFlights[static_cast<uint32_t>(airport)] = 0;
  }

  const static uint32_t airportsCount = static_cast<uint32_t>(TAirport::Last_);
  using Item = pair<TAirport, size_t>;
  using Items = array<Item, airportsCount>;
  ...
  Items GetItems() const
  {
      ...
      {
          output[i] = make_pair(static_cast<TAirport>(i), airportToFlights.at(i));
      }
      return output;
  }

private:
   array< uint32_t, airportsCount>  airportToFlights;
};
...
void TestMoscow() {
  ...
  auto s = airport_counter.GetItems();
